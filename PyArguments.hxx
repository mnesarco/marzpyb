...
template <typename T>
concept has_clean_method = requires(std::tuple<int>& t) {
    { T::template clean<0>(t) } -> std::same_as<void>;
};

template <typename T>
concept has_parse_ptr_value = requires(T t) { T::parse_ptr_value(); };

// Check if two types are the same, ignoring cv-qualifiers and references
template <typename T1, typename T2>
inline constexpr bool is_same_type_v
...
inline void apply_init_helper(Parsed& parsed, const std::tuple<Args...>* args = nullptr)
{
    if constexpr (Index < sizeof...(Args))
    {
        ...
        if constexpr (has_default_value<arg_t>)
        {
...
inline void apply_clean_helper(Parsed& parsed, const std::tuple<Args...>* args = nullptr)
{
    ...
    {
        using arg_t = std::tuple_element_t<Index, std::tuple<Args...>>;
        if constexpr (has_clean_method<arg_t>)
        {
...
inline constexpr auto parse_ptr(T&& obj)
{
    ...
    if constexpr (has_parse_ptr_value<Type>)
    {
...
template <typename... Fmts>
inline consteval auto fmt_concat(Fmts... fmts)
{
    FmtString<fmt_size<Fmts...>> result {};
...
{
    static_assert(detail::has_parse_ptr_value<Encoding>,
